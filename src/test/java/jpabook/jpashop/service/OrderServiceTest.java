...
class OrderServiceTest {
    ...
    public void 상품주문_재고수량초과() throws Exception {
        //given
        Member mmeber = createMember();
        Book book = createBook("시골 JPA",10000, 10);

        int orderCount = 11;
        //when
        //then
        Assertions.assertThatThrownBy(() -> orderService.order(mmeber.getId(), book.getId(), orderCount))
    ...
    private Member createMember() {
        ...
        member.setName("회원1");
        member.setAddress(new Address("서울", "하하", "123-123"));
        em.persist(member);
        return member;
    }
